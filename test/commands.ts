        ...
        forEachPromise(commands.DO_NOTHING_COMMANDS, (text: string) => {
            return new Promise<void>((resolve) => {
                commands
                    ...
                    .then((result) => {
                        //console.log(result.debug.classification);
                        expect(result.command, 'Tested-command: "' + text + '"').to.equal(commands.COMMANDS.DO_NOTHING);
                        //expect(result.options, 'Tested-command: "' + text + '"').to.deep.equal({});
                        resolve();
                    })
                    .catch((err) => done(err));
        ...
        forEachPromise(commands.CREDS_COMMANDS, (text: string) => {
            return new Promise<void>((resolve) => {
                commands
                    ...
                    .then((result) => {
                        //console.log(result.debug.classification);
                        expect(result.command, 'Tested-command: "' + text + '"').to.equal(commands.COMMANDS.SEND_CREDS);
                        //expect(result.options, 'Tested-command: "' + text + '"').to.deep.equal({});
                        resolve();
                    })
                    .catch((err) => done(err));
